...
class PlotValidationPredictionsCallback(BestEpochCallback):
    ...
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        ...
        # y = y.cpu().numpy()
        self.ground_truths = y
        self.predictions = predictions
        # self.ground_truths.append(y[:, 0, :])
        # self.predictions.append(predictions[:, 0, :])

    def on_fit_end(self, trainer, pl_module):
        ground_truth = np.array(self.ground_truths)
        predictions = [x.numpy()[0] for x in self.predictions]
        ...
        plt.xlabel("Game Match")
        plt.ylabel("Score Difference")
        # tensorboard.add_figure(
        #     "Prediction result of game match",
        #     fig,
