...
def nba_cross_entropy_with_player_name(inputs, targets, model, lamda=1.5e-3):
    assert inputs.shape[0] == targets.shape[0]
    leng = inputs.shape[0]
    acc = 0.0
    game = 0.0
    ce = 0.0
    p = torch.tensor([], requires_grad=True)
    y = []
    for i in range(leng):
        inp = inputs[i]
        t = torch.reshape(targets[i], (targets[i].shape[1], targets[i].shape[2]))
        for j in range(t.shape[0]):
            if t[j][0] != 0 or t[j][1] != 0:
                ...
                r_y = model.regressor4(r_y)
                if False in torch.isnan(r_y):
                    r_y_c = softmax(r_y[0].item(), r_y[1].item())
                    if t[j][2] > 0:
                        p1 = r_y_c[0][0]
                        eps = 1e-15
