...
def nba_rmse_with_player_with_regularizer_loss(inputs, targets, model, team_2_player, lamda=1.5e-3, isMean=True):
    assert inputs.shape[0] == targets.shape[0]
    leng = inputs.shape[0]
    game = 0.0
    rmse_loss = 0.0
...
def nba_output_with_player(inputs, targets, model, team_2_player, isMean=True):
    assert inputs.shape[0] == targets.shape[0]
    leng = inputs.shape[0]
    p, y = list(), list()
    if isMean:
