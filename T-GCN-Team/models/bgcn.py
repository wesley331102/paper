...
class BGCNCell(nn.Module):
    def __init__(self, adj: np.ndarray, adj_1: np.ndarray, adj_2: np.ndarray, adj_3: np.ndarray, adj_4: np.ndarray, adj_5: np.ndarray, team_2_player: dict, input_dim_t: int, input_dim_p: int, aspect_num: int, feature_dim: int, hidden_dim: int, co_attention_dim: int, applying_team: bool, applying_player: bool, applying_co_attention: bool):
        super(BGCNCell, self).__init__()
        # applying GCN
        self._applying_team = applying_team
        # applying RGCN
        self._applying_player = applying_player
        # applying co-attention
        self._applying_co_attention = applying_co_attention
        ...
        if (self._applying_team and self._applying_player) and self._applying_co_attention:
            # Co-attention
    ...
    def forward(self, inputs, hidden_state, seq_index):
        if self._applying_team and self._applying_player:
            if self._applying_co_attention:
                ...
                player_inputs = inputs[:, self._input_dim_t:, :]
                # batch size * (num of player nodes * hidden state dimension))
                batch_dim_p, _, _ = player_inputs.shape
                assert batch_dim_p == len(seq_list)
                # batch size * (num of team nodes * hidden state dimension)
                team_hidden_state = hidden_state[:, :self._input_dim_t*self._hidden_dim]
                # batch size * (num of player nodes * hidden state dimension)
                player_hidden_state = hidden_state[:, self._input_dim_t*self._hidden_dim:]
                ...
                # batch size * (num of nodes * hidden state dimension)
                new_hidden_state = torch.cat((new_hidden_state_t, new_hidden_state_p), 1).reshape(batch_dim_p, ((self._input_dim_t + self._input_dim_p) * self._hidden_dim))
...
class BGCN(nn.Module):
    def __init__(self, adj: np.ndarray, adj_1: np.ndarray, adj_2: np.ndarray, adj_3: np.ndarray, adj_4: np.ndarray, adj_5: np.ndarray, team_2_player: dict, aspect_num: int, hidden_dim: int, co_attention_dim: int, applying_team: bool, applying_player: bool, applying_co_attention: bool, applying_attention: bool, aspect_dim: int = 16, **kwargs):
    ...
    def add_model_specific_arguments(parent_parser):
        ...
        parser.add_argument("--applying_co_attention", action="store_true")
    ...
    def hyperparameters(self):
        return {
            ...
            "applying_co_attention": self._applying_co_attention,
